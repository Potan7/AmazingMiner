...
namespace CoreDriller.Player.StatSystem
{
    ...
    {
        // 연료 (현재값, 최대값)
        public static readonly Subject<(float current, float max)> Fuel = new();
        // 체력 (현재값, 최대값)
        public static readonly Subject<(float current, float max)> Health = new();
        // 인벤토리 변경 알림
        public static readonly Subject<Unit> InventoryChanged = new();
